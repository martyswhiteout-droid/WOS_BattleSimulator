...
| How do I deploy the shell (with OCR) to a VPS as password-gated private staging, pre-Clerk? | `docs/VPS_DEPLOY_RUNBOOK.md` (2026-08-29; Hostinger KVM2 + Docker + Caddy basic_auth stopgap) |
...
| `ENGINE_HANDOFF_winprob_surface_what_varies.md` | Why the headline win% sits at exactly 25%/75% for every near-even (garrison) scenario: one-bit display collapse x `def_k=0.45` stuck bit x band-edge cliff/non-monotone; 35-variation repro, G12 check of `def_k=1.0` (6/13, gate-blocked), proposed fix = surface hold rate / casualty margin / strength ratio / branch + continuous call-stability headline | 12 KB, medium | **IMPLEMENTED 2026-09-10** (commit `41a17ea`: call-stability headline + 'what moved' strip; G12 7/13, Brier 0.2011) |
| `wos_sim/formula_research/EXPERIMENT_DT_STACKING.md` + `dt_stacking_ladder.py` | PRE-REGISTERED experiment: how the game composes k copies of a Damage-Taken joiner (Wu Ming −25%): additive / multiplicative / engine-today (gamma 0.30) / capped, frozen prediction table + ±5% decision rule; Gatot-instrument design (the DT side must die), control ladder for joiner troops; triggered by `Gen15_Rally_WM` (3× WM holds 99/100 in the sim) | 6 KB, short | **ENGINE CHANGED 2026-09-16** — `TURN_PARAMS["modifier_stacking"]="multiplicative"` (DT measured, DD by symmetry; G12 7/13, Brier 0.1817); max-level rungs infeasible (owner) |
