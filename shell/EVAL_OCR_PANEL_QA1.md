...
**History note (2026-08-11):** commit `fd7b631` (message: 'research: cross-class army law (Stage 8.1)') ALSO contains the D-040 overlay sheet-accessibility implementation (4 UI files) — a session-recovery mixup swept pending UI edits into a commit meant for unrelated research work. Left un-rewritten deliberately (landed history); D-040's tests/follow-up live in `b2d7d9d`.

# L4 GATE CLOSED (2026-08-11, HEAD 6cd9b8e+2082210) — overlay UI READY

Real-app L4 rounds found D-035..D-042 (dead Next, discarded enemy column, wrong-variant routing, recovery desync+trap, missing thumbnails, sheet a11y, manual-path fill, S5 editor wiring) — all fixed and probe-verified. Final: headline criterion measured (real C_battle_3+4 upload -> 1 network call -> 24/24 converted scout-net values digit-exact in #statPanel, statsScouted true; typed overrides win per-field); recovery, manual floor, Undo/Reset, provenance, sheets, both widths all pass; suites 341/144/7 twice. Evaluator: 'That is the feature as specified, and it is ready to ship.' Documented nit (unfixed, cosmetic): the recovery notice persists after a successful replacement upload. Remaining before RELEASE (not gate items): merge decision, shell fix-round (F-criticals + this endpoint behind real auth), adapter-QA debt round, THIRD_PARTY_SETUP account registrations, perf budgets under real VPS load.
