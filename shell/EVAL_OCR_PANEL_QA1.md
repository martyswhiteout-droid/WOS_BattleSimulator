...
- **Standing open (by design, unchanged):** L2 real-image benchmark (owner PNGs pending — the largest unverified risk; NO confidence claim about real screenshots is supported yet) · L4 overlay UI unbuilt · perf budgets (no engines) · `/shell/ocr/panel` unmetered (`limits.py OCR_ENDPOINTS` exact-match — shell fix-round item) · bounded documented residuals: >1MiB sub-ceiling uploads spool to OS temp for request lifetime; absent-Content-Length fails open to post-parse caps.
