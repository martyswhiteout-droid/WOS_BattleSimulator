...
- [ ] **D2.** Rate limiting, layered: per-account daily query budget (free tier: 5 sims/day; paid: 100 sims/day — reconciled 2026-08-04 per COMPASS D2, lower pair wins; the `entitlements` table is the single source of truth), per-account burst limit (≤ 5/min), per-IP limits, and a global concurrency cap so a flood degrades gracefully instead of taking the service down (DoS resilience).
