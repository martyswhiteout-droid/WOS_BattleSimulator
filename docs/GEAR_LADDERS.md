...
- **REVISED U MODEL (Marlinman discriminates; supersedes the §6 story in STAT_PANELS_FORMULA for the Leth/HP half):**
  `U_AtkDef(class) = hero_card_injection(gen, ascension) + gate points` — Rufus 5★: 1281.02 table + 40 = 1323.05 observed ✓ exact; Gisela 4★+1 / Karol 4★+2 close to 0.15–0.19% via the ascension law below.
- **ASCENSION LAW (decoded 2026-08-07 from 7 owner Promotion-Preview screens):** hero-card Expedition Atk=Def = `gen_max × fraction(star, shard)`. Per-shard increment = `gen_max × s(star)` with **s ≈ 1.644% × 1.4^(star−1)** (measured: 1★ 1.644%, 2★ 2.298%, 3★ 3.217%, 4★ 4.506–4.511% — gen-INVARIANT across g11/g12/g13/g15). Star-base fractions are UNIVERSAL: measured 1★+3sh 25.53%, 2★ 31.775%, 2★+1 34.07%, 4★ **69.329%** (two gens exactly), 4★+1 73.84%, 5★ = 100% (= HERO_GEN_STAT). Marlinman closure: Gisela pred 1197.2 vs 1199.05 implied (0.15%), Karol 1136.8 vs 1139.0 (0.19%), Rufus exact. **The hero card has NO Expedition Leth/HP lines at all** (Promotion Preview shows only Atk+Def) — independent proof of the widget reversal. Residuals: displayed increments are rounded (~2pp drift over shards); shard-count arithmetic implies an extra jump at star-up itself (one before/after ascension capture would pin it); unmeasured cells (3★ base, shards-per-star).
