...
- **Mastery** (number ON the gear, `Lv.N`): +10%/level of the gear's stat; max Lv.20 = 200%.
- **Enhancement** (top-right `+N`): +1%/level to the gear's Leth-or-HP stat, max +100. Gates at +20/+60/+100 add **Expedition Atk/Def** per the owner's slot table (+20 gate = +20%, +60 gate = +30%, +100 gate = +50%; slot alternation makes a fully-gated hero grant exactly **+200 Atk and +200 Def** to its class). +40/+80 gates = Exploration (hero-only) stats — no troop panel impact.
- Whale reconciliation ✓: max hero gear = (200+100) ×2 leth gears = +600 Leth, ×2 hp gears = +600 HP, gates = +200/+200 Atk/Def — matches the measured whale U residual (605/605/205/205 = these + the uniform +5).
- **Widget level** = `+N` on the first-row leftmost icon. **Widget SKILL scales with level** (PROVEN: Marlinman +7.5% Defender Troops' Attack vs whales' +15% Atk+HP) — fold sets must be read per report. **Widget BASE stats (e.g. Helacore +5 = Inf Leth/HP +202.5%) are NOT in the panel class rows** (proven: they'd force negative injections on Account C).
