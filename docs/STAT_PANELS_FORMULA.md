...
2. **U decomposition residuals** (§6, updated after §9; REVISED 2026-08-07 by the third account + the mastery law — see `docs/GEAR_LADDERS.md` §3): experts-in-BO is MEASURED (two-account discriminator). The U internals are now: `U_AtkDef = hero-card injection (gen, ascension-scaled) + gear gate points`; `U_LethHP = gear (100+E)(100+10M)/200 arithmetic + widget base stats` — hero cards contribute NO Leth/HP; `HERO_GEN_STAT`'s Leth/HP column was numerically absorbing the widgets and needs re-derivation. Remaining open: the Atk/Def-side small extra (whales +5.00; alt +15.00), the ascension ladder (needs star counts), per-widget base-stat ladders.
