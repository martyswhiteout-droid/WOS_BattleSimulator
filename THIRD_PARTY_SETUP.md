...
| 4 | **Stripe** — stripe.com | Create account (HK supported). Stay in TEST mode. Create product "WOS Tests Pro" US$7.99/mo (placeholder — change freely). Add webhook endpoint later when staging URL exists; copy signing secret | `STRIPE_SECRET_KEY`, `STRIPE_WEBHOOK_SECRET`, `STRIPE_PRICE_ID_PRO` | 2.9%+fees | Paddle (merchant of record — handles global tax), Airwallex |
